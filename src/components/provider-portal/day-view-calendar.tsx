      ...
      <div className="border-r last:border-r-0">
        ...
        <div className={`relative h-full divide-y`}>
          ...
          {appointments.map(apt => {
            ...
            return (
                <div key={apt.name} className="absolute inset-x-2" style={{ top: `calc(${apt.start * 4}rem + 2px)`, height: `calc(${apt.duration * 4}rem - 4px)`}}>
                    <div className={`p-2 rounded-lg border h-full ${colorSet[apt.color as keyof typeof colorSet]}`}>
