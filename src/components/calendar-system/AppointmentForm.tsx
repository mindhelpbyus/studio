...
export function AppointmentForm({
  ...
  const [formData, setFormData] = useState({
    ...
    notes: '',
    color: '', // Changed to string | undefined
  });
  ...
  const initializeForm = () => {
    if (editingAppointment) {
      ...
      setFormData({
        ...
        status: (editingAppointment.status === 'confirmed' || editingAppointment.status === 'pending') ? editingAppointment.status : 'confirmed',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const appointmentData: Omit<Appointment, 'id'> = {
        ...
        color: formData.color || selectedTherapist?.color || '#3b82f6',
        createdBy: 'therapist',
        notes: formData.notes,
        clientName: selectedClient?.name || '',
        therapistName: selectedTherapist?.name || '',
        serviceId: '', // Added missing property
        patientName: selectedClient?.name || '', // Added missing property
  ...
  const handleDurationChange = (duration: string) => {
    const [startHoursStr, startMinutesStr] = formData.startTime.split(':');
    const startHours = parseInt(startHoursStr || '0', 10);
    const startMinutes = parseInt(startMinutesStr || '0', 10);
