...
const generateMockAppointments = (): Appointment[] => {
  ...
  for (let dayOffset = -7; dayOffset < 30; dayOffset++) {
    ...
    for (let i = 0; i < appointmentsPerDay; i++) {
      ...
      endTime.setMinutes(startTime.getMinutes() + (duration as number));
      ...
      if (type === 'break') {
        ...
        title = breakTypes[Math.floor(Math.random() * breakTypes.length)] as string;
        clientName = '';
      } else if (type === 'tentative') {
        title = `Tentative: ${client?.name || 'Unknown Client'}`;
        clientName = client?.name || 'Unknown';
      } else {
        title = `Session with ${client?.name || 'Unknown Client'}`;
        clientName = client?.name || 'Unknown';
      }
      ...
      appointments.push({
        ...
        clientId: type !== 'break' ? client?.id || '' : '',
        serviceId: '', // Added missing property
        title,
        startTime: startTime,
        endTime: endTime,
        status: Math.random() > 0.2 ? 'confirmed' : 'pending',
        type,
        patientName: clientName, // Added missing property
        color: type === 'break' ? '#6b7280' : therapist?.color || '#3b82f6',
        isDraggable: true, // Added missing property
        isResizable: true, // Added missing property
        createdBy: Math.random() > 0.7 ? 'patient' : 'therapist',
        notes: type === 'break' ? '' : `Notes for ${clientName}`, // Ensure notes is always a string
        clientName,
...
export const appointmentsApi = {
  async getAppointments(startDate?: string, endDate?: string, therapistIds?: string[]): Promise<Appointment[]> {
    ...
    if (startDate) {
      ...
      filtered = filtered.filter(apt => apt.startTime >= filterStartDate);
    }
    
    if (endDate) {
      const filterEndDate = new Date(endDate);
      filtered = filtered.filter(apt => apt.startTime <= filterEndDate);
    }
  ...
  async updateAppointment(id: string, updates: Partial<Appointment>): Promise<Appointment> {
    ...
    mockAppointments[index] = { ...mockAppointments[index], ...updates } as Appointment;
